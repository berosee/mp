        ...
        if (Array.isArray(data)) {
            data.forEach((file) => {
                if (file.name.endsWith(".mp3") || file.name.endsWith(".flac")) {
                    ...
                    // 1. 한글 파일명이 주소에서 깨지지 않도록 안전하게 인코딩합니다.
                    const safeName = encodeURIComponent(file.name);
                    let url = "";

                    // 2. [핵심] 파일 크기가 20MB(20,971,520 바이트) 이하인 경우만 jsDelivr 가속 사용
                    if (file.size && file.size <= 20971520) {
                        url = `https://cdn.jsdelivr.net/gh/${user}/${repo}@main/${folder}/${safeName}`;
                    } else {
                        // 3. 20MB를 초과하는 대용량 파일은 jsDelivr 차단을 피해 GitHub 원본 직링크로 연결
                        url = `https://raw.githubusercontent.com/${user}/${repo}/main/${folder}/${safeName}`;
                    }
